...
class CheckNetwork(object):
    ...
    def report_fail(self):
        ...
        if self.continue_fail_count > 10:
            ...
            logger.debug("report_connect_fail %s continue_fail_count:%d",
    ...
    def _test_host(self, url):
        try:
            ...
            if __name__ == "__main__":
                logger.exception("test %s e:%r", url, e)
    ...
    def _simple_check_worker(self):
        ...
        for url in self.urls:
            if self._test_host(url):
                ...
                if __name__ == "__main__":
                    logger.warn("test %s fail", url)
                time.sleep(1)

        if network_ok:
            self.last_check_time = time.time()
            self.report_ok()
            logger.debug("network %s is ok, cost:%d ms", self.type, 1000 * (time.time() - time_now))
        else:
            logger.warn("network %s fail", self.type)
