        ...
        try:
            ...
            if response:
                if response.status > 600:
                    logger.warn("direct %s %s % status:%d", method, host, path, response.status)
                    continue
                    ...
                    if "G" not in server_type and "g" not in server_type and server_type not in google_server_types:
                        ...
                        logger.warn("IP:%s host:%s not support GAE, server type:%s status:%d",
            ...
            errors.append(e)
            logger.exception('direct_handler.handler %r %s %s , retry...', e, host, path)
