...
class CertUtil(object):
    ...
    def create_ca():
        ...
        #logger.debug("CA key:%s", key)
        logger.info("create CA")
        return key, ca

    @staticmethod
    def generate_ca_file():
        logger.info("generate CA file:%s", CertUtil.ca_keyfile)
    ...
    def generate_cert_keyfile():
        logger.info("generate certs's key file:%s", CertUtil.cert_keyfile)
        ...
        if os.path.exists(certfile):
            ...
            if datetime.datetime.strptime(cert.get_notAfter(), '%Y%m%d%H%M%SZ') < datetime.datetime.utcnow() + datetime.timedelta(days=30):
                try:
                    ...
                    logger.warning('CertUtil._get_old_cert failed: unable to remove outdated cert, %r', e)
                else:
                    return
    ...
    def import_windows_ca(certfile):
        logger.debug("Begin to import Windows CA")
        with open(certfile, 'rb') as fp:
            certdata = fp.read()
            ...
            try:
                ...
                if not store_handle:
                    if store == CERT_SYSTEM_STORE_CURRENT_USER and not ca_exists:
                        logger.warning('CertUtil.import_windows_ca failed: could not open system cert store')
                        return False
                    else:
                        continue
                ...
                while pCertCtx:
                    ...
                    if _certdata == certdata:
                        ca_exists = True
                        logger.debug("XX-Net CA already exists")
                    else:
                        ...
                        if cert_name == common_name:
                            ...
                            if ret == 1:
                                logger.debug("Invalid Windows CA %r has been removed", common_name)
                ...
                logger.warning('CertUtil.import_windows_ca failed: %r', e)
    ...
    def get_linux_firefox_path():
        ...
        for filename in os.listdir(firefox_path):
            if filename.endswith(".default") and os.path.isdir(os.path.join(firefox_path, filename)):
                ...
                #logger.debug("Got Firefox path: %s", config_path)
                return config_path

    @staticmethod
    def import_linux_firefox_ca(common_name, ca_file):
        logger.debug("Begin importing CA to Firefox")
        firefox_config_path = CertUtil.get_linux_firefox_path()
        if not firefox_config_path:
            #logger.debug("Not found Firefox path")
            return False

        if not any(os.path.isfile('%s/certutil' % x) for x in os.environ['PATH'].split(os.pathsep)):
            logger.warn('please install *libnss3-tools* package to import GoAgent root ca')
            return False

        logger.info("Removing old cert to Firefox in %s", firefox_config_path)
        ...
        logger.info("Add new cert to Firefox in %s", firefox_config_path)
    ...
    def import_linux_ca(common_name, ca_file):
        ...
        if not any(os.path.isfile('%s/certutil' % x) for x in os.environ['PATH'].split(os.pathsep)):
            logger.info('please install *libnss3-tools* package to import GoAgent root ca')
            return False

        sha1 = get_linux_ca_sha1(nss_path)
        ca_hash = CertUtil.ca_thumbprint.replace(':', '')
        if sha1 == ca_hash:
            logger.info("Database $HOME/.pki/nssdb cert exist")
            return
        ...
        logger.info("Removing old cert in database $HOME/.pki/nssdb")
        cmd_line = 'certutil -L -d sql:$HOME/.pki/nssdb |grep "GoAgent" && certutil -d sql:$HOME/.pki/nssdb -D -n "%s" ' % ( common_name)
        os.system(cmd_line)

        # install new cert
        logger.info("Add cert to database $HOME/.pki/nssdb")
    ...
    def import_ubuntu_system_ca(common_name, certfile):
        ...
        if not os.path.exists(pemfile) or not CertUtil.file_is_same(certfile, new_certfile):
            if os.system('cp "%s" "%s" && update-ca-certificates' % (certfile, new_certfile)) != 0:
                logger.warning('install root certificate failed, Please run as administrator/root/sudo')
    ...
    def init_ca():
        ...
        if not os.path.exists(CertUtil.ca_keyfile):
            if os.path.exists(CertUtil.ca_certfile):
                ...
                logger.info("update CA file storage format")
                if hasattr(OpenSSL.crypto, "X509StoreContext"):
                    os.rename(CertUtil.ca_certfile, CertUtil.ca_keyfile)
                else:
                    logger.warning("users may need to re-import CA file")
                    CertUtil.generate_ca_file()
            else:
                logger.info("no GAE CA file exist in XX-Net data dir")

                logger.info("clean old site certs in XX-Net cert dir")
